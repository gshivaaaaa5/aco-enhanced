...
{
    TypeHeaderTest()
        : TestCase("ACO TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(ACOTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), ACOTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(ACOTYPE_RREP);
 ...
 * @ingroup aco-test
 *
 * @brief Unit test for ACO routing table entry
 */
...
struct AcoRqueueTest : public TestCase
{
    AcoRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
AcoRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&AcoRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&AcoRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &AcoRqueueTest::CheckTimeout, this);
 ...
 * @brief Unit test for ACO routing table entry
 */
struct AcoRtableEntryTest : public TestCase
{
    AcoRtableEntryTest()
 ...
 * @ingroup aco-test
 *
 * @brief Unit test for ACO routing table
 */
struct AcoRtableTest : public TestCase
{
    AcoRtableTest()
        : TestCase("Rtable")
    {
 ...
 * @ingroup aco-test
 *
 * @brief ACO test suite
 */
class AcoTestSuite : public TestSuite
{
  public:
    AcoTestSuite()
        : TestSuite("routing-aco", Type::UNIT)
    {
        ...
        AddTestCase(new AcoRqueueTest, TestCase::Duration::QUICK);
        AddTestCase(new AcoRtableEntryTest, TestCase::Duration::QUICK);
        AddTestCase(new AcoRtableTest, TestCase::Duration::QUICK);
    }
} g_acoTestSuite; ///< the test suite
