 ...
 * @ingroup aco-test
 *
 * @brief ACO regression test suite
 */
class AcoRegressionTestSuite : public TestSuite
{
  public:
    AcoRegressionTestSuite()
        : TestSuite("routing-aco-regression", Type::SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("aco-chain-regression-test"),
...
} g_acoRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("ACO chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & ACO
    AcoHelper aco; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(aco);
    ...
    constexpr int expectedAcoStreamsPerDevice = 1;
    ...
    NS_TEST_ASSERT_MSG_EQ(
        ...
        "Stream assignment mismatch");
    streamsUsed += aco.AssignStreams(*m_nodes, streamsUsed);
    // ACO uses numDevices * expectedAcoStreamsPerDevice
    NS_TEST_ASSERT_MSG_EQ(
        streamsUsed,
        (devices.GetN() * (expectedWifiStreamsPerDevice + expectedInternetStreamsPerDevice +
                           expectedAcoStreamsPerDevice)),
        "Stream assignment mismatch");
